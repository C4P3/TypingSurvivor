...
- [ ] **UIナビゲーションシステムの本格実装**
    - [ ] `UIManager`を改修し、パネルのスタック管理と、`PushPanel`/`PopPanel`メソッドを実装する。
    - [ ] `MainMenuManager`をリファクタリングし、`UI-Design.md`に記載された画面フロー（タイトル→メインメニュー→各種パネルのオーバーレイ表示・非表示）を完全に実装する。
    - [ ] ランキング、設定、ショップ、クレジット画面のプレースホルダー（空のパネル）を実装し、メインメニューからの遷移を実装する。
