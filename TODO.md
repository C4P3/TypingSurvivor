...
- [ ] **設定機能の実装**:
  - [ ] **設計**: `Settings-Design.md` を作成し、キーコンフィグと音量バランス調整機能の全体設計を行う。
  - [ ] **データ構造**: `PlayerSaveData.cs` を拡張し、音量(BGM/SFX)とキーバインド設定を保存するフィールドを追加する。
  - [ ] **音量調整機能**:
    - [ ] `MusicManager` と `SfxManager` を修正し、個別の音量と全体設定の音量を乗算して適用するロジックを実装する。
  - [ ] **キーコンフィグ機能**:
    - [ ] `InputSystem` のリバインド機能を利用して、キー設定を変更・保存・復元する `SettingsManager` を作成する。
  - [ ] **UI実装**:
    - [ ] メインメニューから遷移できる設定画面のUIプレハブと、それを制御する `SettingsScreen` スクリプトを作成する。
  - [ ] **データ永続化**:
    - [ ] `CloudSaveService` を拡張し、新しい設定データをクラウドに保存・ロードする処理を実装する。
...
## ✅ 完了済みタスク (Completed)
- [x] **通知システムの刷新と切断処理の堅牢化**
  - `NotificationPanel`を廃止し、役割の異なる`TopNotificationPanel`（一時通知用）と`DisconnectNotificationScreen`（モーダル画面）に分割して再設計した。
  - ゲーム中、リザルト中などフェーズを問わず相手の切断が通知されるようにした。
  - プレイヤー切断時もゲームが正しく終了し、レート計算が実行され、リザルト画面で切断情報が表示されるように修正した。
...
- [x] **リザルト画面作成**
  ...
  - 再戦とメインメニューのボタンの画面
- [x] 爆弾やロケットなどの一部のアイテムのエフェクトが消えないバグの修正
