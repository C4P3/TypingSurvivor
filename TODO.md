...
- [ ] **UI詳細設計の実装 (MainMenu)**
    - [ ] `UIManager`に、パネルを重ねて表示する`ShowPanelOverlay`/`HidePanelOverlay`メソッドを実装する。
    - [ ] `MainMenuManager`をリファクタリングし、`UI-Design.md`に記載された画面フロー（タイトル→メインメニュー→各種パネル）を実装する。
    - [ ] ランキング、設定、ショップ、クレジット画面の、ボタンやラベルが配置された仮のパネルを作成し、遷移できるようにする。
