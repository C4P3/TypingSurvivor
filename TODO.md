...
## 優先度: 高
- [ ] 相手が部屋を退出したときに、どのモードであっても、通知パネルで相手の退出を伝えるようにしたい。
- [ ] 相手が退出して、十分な人数がいないときに、リザルトパネルでのテキストメッセージを適切なものに修正する。
- [ ] NotificationPanelが機能していないので修正する。また、通知としてのクライアント切断情報と、メインメニューボタンが必須なサーバーとの切断情報は完全に別のパネルで別のデザインで実装する。
- [ ] 相手が退出したときも、リザルトでレート変動をちゃんと表示する。
