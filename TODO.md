...
ゲーム体験を向上させるため、サウンドとVFX（ビジュアルエフェクト）の表現力を強化するリファクタリングと機能追加を行う。

- [ ] **コアVFXシステムの機能拡張**
    - [ ] **持続・追従エフェクト**: `EffectManager`に`PlayAttachedEffect(target, duration)`を実装し、特定のオブジェクトに追従して一定時間表示されるエフェクトを再生できるようにする。
        - [ ] `VFXAutoDestroyWithDuration.cs`コンポーネントを新規作成する。
    - [ ] **指向性エフェクト**: `EffectManager.PlayEffect`を拡張し、回転とスケールを指定できるようにする。これにより、ロケットのような直線状のエフェクトの長さと方向を動的に制御できるようにする。
- [ ] **各種IDの追加**
    - [ ] `AudioRegistry.cs`の`SoundId`に、ロケット、回復、加速、スター、毒、うんち用のIDを追加する。
    - [ ] `VFXRegistry.cs`の`VFXId`に、上記アイテムに対応するIDを追加する。
- [ ] **アイテムエフェクトの実装と修正**
    - [ ] **スター**: `StarEffect.cs`を修正し、新しい`PlayAttachedEffect`を使って、効果時間中プレイヤーに追従するエフェクトを再生するようにする。
    - [ ] **サンダー**: `StunOpponentEffect.cs`を修正し、`PlayAttachedEffect`を使って、気絶している間、相手プレイヤーに追従するエフェクトを再生するようにする。
    - [ ] **ロケット**: `DirectionalDestroyEffect.cs`を修正し、拡張された`PlayEffect`を使って、破壊する範囲と一致した長さ・方向のエフェクトを再生するようにする。
    - [ ] **回復/加速**: `OxygenHealEffect.cs`と`MoveSpeedupEffect.cs`に、エフェクトとサウンド再生ロジックを追加する。
    - [ ] **毒**: `DamageOpponentOxygenEffect.cs`に、相手の位置にエフェクトとサウンドを再生するロジックを追加する。
    - [ ] **うんち**: `TransformItemToBlockEffect.cs`に、ブロック設置位置でエフェクトとサウンドを再生するロジックを追加する。
