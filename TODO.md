...
## 🛠️ フェーズ2.5: サウンドシステムのデータ駆動化リファクタリング

現在のサウンド実装を、新しく設計された、より堅牢で拡張性の高いデータ駆動システムに置き換える。

- [ ] **サウンドシステムの分離と専門化**:
    - [ ] **`AudioManager`の改修**: BGM関連の機能をすべて削除し、`SoundEffectData`に基づくSFX再生専用のサービスに特化させる。
    - [ ] **`MusicManager`の新規作成**: BGMの再生、クロスフェード、スタック管理、ジングル遷移を専門に扱う`MusicManager`を、2つのAudioSource（ターンテーブル）を持つ永続的なサービスとして新規に作成する。
- [ ] **データ構造の作成**:
    - [ ] **`SoundEffectData.cs`の作成**: 複数のクリップ、ランダムピッチ、音量などを定義できるSFX用のScriptableObjectを作成する。
    - [ ] **`MusicData.cs`の作成**: AudioClip、音量、ループ設定などを定義できるBGM用のScriptableObjectを作成する。
- [ ] **`AudioRegistry`の更新**:
    - [ ] `SoundId`と`SoundEffectData`/`MusicData`を紐付ける、サウンドの「カタログ」として機能するように修正する。
- [ ] **呼び出し元の更新**:
    - [ ] プロジェクト内のサウンド再生箇所を全て見直し、新しい`AudioManager`と`MusicManager`のAPIを正しく呼び出すように修正する。
