...
2.  **Playingフェーズ**:
    *   このフェーズの間、GameManagerは定常的な状態更新を行います。特に、酸素は時間経過と共に**減少速度が加速**していきます。
        *   **0分～10分**: 減少レートは、初期値から10分後の基準値まで**線形に増加**します。
        *   **10分以降**: 10分時点のレートと傾きを維持したまま、**対数（Log）カーブに接続**されます。これにより、レートは上限なく、しかし緩やかに増加し続けます。
...
`Playing`フェーズでは、「時間経過による継続的な酸素減少」と「アイテム取得による瞬間的な酸素増減」のように、複数のロジックが同じフレーム内でプレイヤーの体力を変更しようとすることがあります。

各ロジックが独立して`GameState`を読み書きすると、更新の上書きや計算のズレといった競合状態が発生します。この問題を解決するため、`GameManager`は**更新集約パターン**を採用します。

1.  フレーム内で発生した全ての体力変化量（デルタ）は、一時的な`Dictionary`に記録されます。
2.  `PlayingPhase`の酸素減少や、`AddOxygen`メソッドは、`GameState`を直接変更せず、この`Dictionary`に変化量を加算（または減算）するだけです。
3.  サーバーの`LateUpdate`で、そのフレームに蓄積された全ての変化量を合計し、最終的な値を**一度だけ**`GameState`に書き込みます。

この設計により、更新の順序に依存しない堅牢な状態管理を実現し、クライアントに送信されるデータの一貫性を保証します。
