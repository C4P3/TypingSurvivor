...
[CreateAssetMenu(fileName = "FbmCaveGeneratorWithAdjustments", menuName = "Typing Survivor/Map Generators/fBM Cave Generator with Adjustments")]
public class FbmCaveGeneratorWithAdjustments : ScriptableObject, IMapGenerator
{
    ...
    [Tooltip("Adjusts the sharpness of the noise. >1 increases contrast, making areas more distinct.")]
    [SerializeField] private float _contrast = 1.0f;
    [Tooltip("Adjusts the distribution of tiles. <1 favors later layers (higher index), >1 favors earlier layers (lower index).")]
    [SerializeField] private float _distributionCurve = 1.0f;
        ...
        for (int x = 0; x < _width; x++)
        {
            for (int y = 0; y < _height; y++)
            {
                ...
                for (int i = 0; i < _octaves; i++)
                {
                    ...
                    // Unity's PerlinNoise returns a value in the range [0,1]
                ...
                float rawNoise = totalNoise / maxAmplitude;
                // --- End fBM ---

                // --- NEW: Apply Contrast and Distribution Curve ---
                // Apply Contrast: pushes values away from the center (0.5) to make areas more distinct.
                float contrastedNoise = (rawNoise - 0.5f) * _contrast + 0.5f;
                float clampedNoise = Mathf.Clamp01(contrastedNoise); // Ensure value stays between 0 and 1.

                // Apply Distribution Curve: adjusts the probability of certain layers appearing.
                float finalNoise = Mathf.Pow(clampedNoise, _distributionCurve);
                // --- End Adjustments ---


                // --- 3. Choose tile based on adjusted noise and thresholds ---
