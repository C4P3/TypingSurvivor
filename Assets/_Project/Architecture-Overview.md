...
#### **UIの非同期更新パターン**
ランクマッチのリザルト画面では、この分離の原則をさらに応用しています。ゲーム終了後、まず基本的な勝敗情報だけをクライアントに送り、リザルト画面を**即座に表示**します。時間のかかるレート計算はサーバーで非同期に実行され、完了次第、新しい`ClientRpc`を通じて計算結果がクライアントにプッシュ通知されます。UIは、その通知を受けて初めてレート表示部分を更新します。この「**即時表示、非同期更新**」パターンにより、バックエンドの処理を待つことなく、プレイヤーに即座にフィードバックを返すことができ、UXを大幅に向上させています。
