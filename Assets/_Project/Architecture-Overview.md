...
* **CQRS (コマンド・クエリ責務分離)** の思想: システムへの命令（書き込み/Command）と、情報の取得（読み取り/Query）をインターフェースレベルで明確に分離します（WriterとReader）。これにより、意図しない状態変更を防ぎ、システムの安全性を高めます。
  * **サービスロケーター (Service Locator)**: `IPlayerStatusSystem`（Core機能）のような各種サービスを、中央のアクセスポイントに登録・集約する手法です。本プロジェクトでは、`AppManager`が汎用的な`IServiceLocator`インターフェースを実装し、サービスを保持する「箱」としての役割を担います。また、`AppManager`は実行環境（クライアントか専用サーバーか）を判断し、`ISurvivalLeaderboardService`のようなインターフェースに対して、クライアント用の実装(`ClientSurvivalLeaderboardService`)とサーバー用の実装(`SurvivalLeaderboardService`)のどちらを注入するかを決定する責務も担います。これにより、実行環境ごとに最適な実装を提供しつつ、利用側はインターフェースにのみ依存することができます。
...
*   **Screens & Panels**: UIの各画面（例: `MainMenuScreen`）や、その上に表示されるパネル（例: `MatchmakingPanel`）は、それぞれ独立したコンポーネントとして実装されています。特に、ランキング画面(`RankingScreen`)は、タブで切り替わる各パネル(`RatingRankingPanel`, `SurvivalRankingPanel`)が`RankingPanelBase`を継承するコンポーネントベースの設計を採用しており、各パネルが自身のデータ表示に責任を持つ構造になっています。
